    ...
    return new Promise((resolve) => {
        if (typeof YaGames !== 'undefined') {
            YaGames.init().then(_ysdk => {
                ...
                // Загружаем сохранения после инициализации игрока
                playerAPI.then(player => {
                    ...
                    console.error('[LOG_ERROR] Ошибка инициализации игрока:', error);
                    loadLocalStats();
                });
...
function updateStatsDisplay() {
    ...
    getPlayerName().then(playerName => {
        playerNameInput.value = playerName || gameStats.playerName;
...
function savePlayerName() {
    ...
    if (newName && newName !== gameStats.playerName) {
        // Проверяем, не пытаемся ли мы изменить имя, полученное из аккаунта
        getPlayerName().then(accountName => {
            if (accountName && newName === accountName) {
                showNotification(currentLanguage === 'ru' ?
                    'Это имя из вашего аккаунта' :
                    'This name is from your account');
                return;
            }
...
function loadGameStats() {
    if (isYandexPlatform) {
        playerAPI.then(player => {
            console.log('[LOG_INFO] Игрок загружен для получения данных:', player);

            // Получаем данные игрока
            return player.getData();
        }).then(data => {
            console.log('[LOG_INFO] Получены данные из облака:', data);
            if (data && data.stats) {
                // Сохраняем статистику из облака
                gameStats = { ...gameStats, ...data.stats };

                // Обновляем имя из аккаунта
                return playerAPI;
            } else {
                // Если данных нет, создаем начальную структуру
                console.log('[LOG_INFO] Нет данных в облаке, используется начальная статистика');
                saveGameStats(); // Сохраняем начальную статистику
                return null;
            }
        }).then(player => {
            if (player) {
                // Получаем имя из аккаунта
                return player.then ? player : playerAPI;
            }
            return null;
        }).then(player => {
            if (player) {
                return player.then ? player : Promise.resolve(player);
            }
            return null;
        }).then(player => {
            if (player) {
                ...
                if (playerName && playerName.trim() !== '') {
                    gameStats.playerName = playerName;
                    console.log('[LOG_INFO] Имя игрока установлено из аккаунта:', playerName);
                }
            }
...
function saveGameStats() {
    // Обновляем счетчик разблокированных достижений
    updateUnlockedAchievementsCount();

    if (isYandexPlatform) {
        playerAPI.then(player => {
            return player.setData({ stats: gameStats });
        }).then(() => {
            console.log('[LOG_INFO] Статистика сохранена в облако');
        }).catch(error => {
            console.error('[LOG_ERROR] Ошибка сохранения статистики в облако:', error);
            saveLocalStats();
        });
    } else {
        saveLocalStats();
